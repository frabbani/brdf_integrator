...
real dotr_func(struct expr_func* f, vec_expr_t* args, void* c) { return brdf_cosine; }
...
static struct expr_func brdf_user_funcs[] = {
    {"dotr", dotr_func, NULL, 0}, {"acos", acos_func, NULL, 0}, {"exp", exp_func, NULL, 0},
    {"pow", pow_func, NULL, 0},   {NULL, NULL, NULL, 0},
};
...
int brdf_expr_load(brdf_expr_t* brdf, const char exprfile[], int term) {
  ...
  char* src = NULL;
  char line[256];
  while (fgets(line, sizeof(line), fp)) size += strlen(line);
